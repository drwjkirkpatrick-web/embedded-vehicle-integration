"""
Tests for embedded_vehicle.obd.interface — OBDInterface with mocked python-obd.
"""
...
from embedded_vehicle.obd.interface import ESSENTIAL_PIDS, OBDInterface, PIDSnapshot
...
async def test_obd_start_emits_connected(mock_obd_connection: MagicMock, monkeypatch: Any, event_bus: Any) -> None:
    ...
    from embedded_vehicle.core import bus as global_bus

    monkeypatch.setattr(
        "embedded_vehicle.obd.interface.asyncio.get_event_loop",
        lambda: asyncio.get_event_loop_policy().get_event_loop(),
    )
    monkeypatch.setattr(
        "embedded_vehicle.obd.interface.asyncio.run_coroutine_threadsafe",
        lambda coro, loop: asyncio.ensure_future(coro),
    )
    monkeypatch.setattr(
        "embedded_vehicle.obd.interface.OBDInterface._poll_loop",
        lambda self: None,
    )
    iface = OBDInterface()
    await iface.start()
...
async def test_obd_start_failed_connection(monkeypatch: Any, event_bus: Any) -> None:
    ...
    monkeypatch.setattr(
        "embedded_vehicle.obd.interface.obd.Async",
