"""
Tests for embedded_vehicle.core — EventBus and configuration.
"""
...
from embedded_vehicle.core import Event, EventBus, EventType, bus, setup_logging
from embedded_vehicle.core.config import MainConfig, settings
...
async def test_config_env_override(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import embedded_vehicle.core.config as config_mod
...
async def test_config_path_validator() -> None:
    ...
    cfg = MainConfig(data_dir="/tmp/vehicle_test")  # type: ignore[arg-type]
    assert isinstance(cfg.data_dir, Path)
    assert str(cfg.data_dir) == "/tmp/vehicle_test"
...
async def test_setup_logging_creates_file(mock_config: MainConfig, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "embedded_vehicle.core.settings",
        mock_config,
    )
    setup_logging()
    log_files = list(mock_config.storage.log_dir.glob("vehicle_*.log"))
