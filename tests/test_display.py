"""
Tests for embedded_vehicle.display.joybring — Head-unit integration with mocked hardware.
...
from embedded_vehicle.display.joybring import JoyBringController, TouchPoint, TouchEventType, SteeringWheelButton
...
async def test_joybring_start_stop(joybring_cfg: Any) -> None:
    ...
    with patch("embedded_vehicle.display.hdmi_sink.HDMISink") as mock_hdmi, \
         patch("embedded_vehicle.display.touch_input.TouchInputHandler") as mock_touch:
...
async def test_joybring_no_can_when_disabled(joybring_cfg: Any) -> None:
    ...
    with patch("embedded_vehicle.display.hdmi_sink.HDMISink") as mock_hdmi, \
         patch("embedded_vehicle.display.touch_input.TouchInputHandler") as mock_touch, \
         patch("embedded_vehicle.display.can_bridge.CANBridge") as mock_can:
        mock_hdmi_inst = AsyncMock()
        mock_hdmi_inst.get_preferred_resolution = AsyncMock(return_value=(1024, 600))
        mock_hdmi.return_value = mock_hdmi_inst
        mock_touch.return_value = AsyncMock()
...
async def test_joybring_no_swc_when_disabled(joybring_cfg: Any) -> None:
    ...
    with patch("embedded_vehicle.display.hdmi_sink.HDMISink") as mock_hdmi, \
         patch("embedded_vehicle.display.touch_input.TouchInputHandler") as mock_touch, \
         patch("embedded_vehicle.display.steering_wheel.SteeringWheelController") as mock_swc:
        mock_hdmi_inst = AsyncMock()
        mock_hdmi_inst.get_preferred_resolution = AsyncMock(return_value=(1024, 600))
        mock_hdmi.return_value = mock_hdmi_inst
        mock_touch.return_value = AsyncMock()
