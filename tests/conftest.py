"""
Shared pytest fixtures for embedded-vehicle-integration test suite.
...
from embedded_vehicle.core import EventBus, EventType
from embedded_vehicle.core.config import MainConfig
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    # Mock obd module so embedded_vehicle.obd.interface can be imported in tests
    ...
    # Mock lgpio so embedded_vehicle.gpio.controller can be imported
    if "lgpio" not in sys.modules:
        mock_lgpio = MagicMock()
        mock_lgpio.SET_PULL_DOWN = 1
        sys.modules["lgpio"] = mock_lgpio

    # Mock pyaudio so embedded_vehicle.audio.assistant can be imported
    if "pyaudio" not in sys.modules:
        mock_pyaudio = MagicMock()
        sys.modules["pyaudio"] = mock_pyaudio

    # Mock picamera2 so embedded_vehicle.camera.recorder can be imported
    if "picamera2" not in sys.modules:
        sys.modules["picamera2"] = MagicMock()

    # Mock smbus2 so embedded_vehicle.imu.sensor can be imported
    if "smbus2" not in sys.modules:
        mock_smbus2_mod = MagicMock()
        mock_smbus2_mod.SMBus = MagicMock
        sys.modules["smbus2"] = mock_smbus2_mod

    # Mock serial so embedded_vehicle.gps.tracker can be imported
    if "serial" not in sys.modules:
        sys.modules["serial"] = MagicMock()

    # Mock pynmea2 so embedded_vehicle.gps.tracker can be imported
    if "pynmea2" not in sys.modules:
        sys.modules["pynmea2"] = MagicMock()

    # Mock telegram so embedded_vehicle.telegram.bot can be imported
...
def mock_config(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> MainConfig:
    ...
    cfg.storage.sqlite_path = tmp_path / "vehicle.db"
...
def mock_obd_connection(monkeypatch: pytest.MonkeyPatch) -> MagicMock:
    ...
    monkeypatch.setattr("embedded_vehicle.obd.interface.obd", mock_obd_mod)
...
def mock_serial(monkeypatch: pytest.MonkeyPatch) -> MagicMock:
    ...
    monkeypatch.setattr("embedded_vehicle.gps.tracker.serial", mock_serial_mod)
...
def mock_pynmea2(monkeypatch: pytest.MonkeyPatch) -> MagicMock:
    ...
    monkeypatch.setattr("embedded_vehicle.gps.tracker.parse_nmea", _fake_parse)
    return mock_mod
...
def mock_telegram_app(monkeypatch: pytest.MonkeyPatch) -> MagicMock:
    ...
    monkeypatch.setattr("embedded_vehicle.telegram.bot.Application", mock_application_cls)
    return mock_app
...
def mock_pyaudio(monkeypatch: pytest.MonkeyPatch) -> MagicMock:
    ...
    monkeypatch.setattr("embedded_vehicle.audio.assistant.pyaudio", mock_mod)
    return mock_instance
