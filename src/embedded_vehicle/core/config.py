"""
embedded_vehicle/core/config.py
───────────────────────
Pydantic-based configuration for the Embedded-Vehicle integration.
...
class OBDConfig(BaseSettings):
    ...
    # 1996 vehicle uses ISO 9141-2 (pin 7) or KWP2000. Not CAN.
...
class CameraConfig(BaseSettings):
    ...
    # Encryption
    encrypt: bool = True
    encryption_key_path: Path = Path("/etc/embedded-vehicle/video.key")
...
class IMUConfig(BaseSettings):
    ...
    enabled: bool = True
    i2c_bus: int = 1  # Pi I2C bus 1 (GPIO 2/3)
...
class GPIOConfig(BaseSettings):
    """GPIO relay and sensor pin assignments for Pi."""
...
class StorageConfig(BaseSettings):
    ...
    # Database
    sqlite_path: Path = Field(default_factory=lambda: Path("/mnt/nvme0n1p2/vehicle.db"))
    # Video
    video_partition: Path = Path("/mnt/nvme0n1p2/video")
    # Encryption
    luks_enabled: bool = True
    luks_keyfile: Path = Path("/etc/embedded-vehicle/luks.key")
