"""
embedded_vehicle/cli.py
───────────────
Command-line tools for individual subsystem testing and debugging.

Entry points (from pyproject.toml):
    vehicle-obd     — Live OBD-II PID monitor
    vehicle-camera  — Camera test / snapshot capture

Usage:
    vehicle-obd --port /dev/ttyUSB0 --watch rpm,speed,coolant
    vehicle-camera --snapshot --camera front
"""
...
from embedded_vehicle.camera.picamera2_backend import PiCamera2Backend
...
logger = logging.getLogger("vehicle.cli")
...
# ── vehicle-status (bonus) ─────────────────────────────────────────────────

def status_cli() -> None:
    """Quick system status print."""
    print("Hermes Vehicle Integration — System Status")
    print("=" * 40)
