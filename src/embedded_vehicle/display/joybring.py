"""
embedded_vehicle/display/joybring.py
────────────────────────────
JoyBring / aftermarket Android head-unit integration controller.

Manages the Pi ↔ head-unit link over HDMI + USB touch back-channel.
...
if TYPE_CHECKING:
    from embedded_vehicle.core import Event
    from embedded_vehicle.core.config import DisplayConfig

logger = logging.getLogger("vehicle.display.joybring")
...
class JoyBringController:
    ...
    async def start(self) -> None:
        ...
        from embedded_vehicle.display.hdmi_sink import HDMISink
        ...
        from embedded_vehicle.display.touch_input import TouchInputHandler
        ...
        if self.cfg and getattr(self.cfg, "can_enabled", False):
            from embedded_vehicle.display.can_bridge import CANBridge
        ...
        if self.cfg and getattr(self.cfg, "swc_enabled", False):
            from embedded_vehicle.display.steering_wheel import SteeringWheelController
