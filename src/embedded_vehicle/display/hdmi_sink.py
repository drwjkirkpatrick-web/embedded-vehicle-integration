"""
embedded_vehicle/display/hdmi_sink.py
─────────────────────────────
HDMI output management for Pi → JoyBring head unit.
...
logger = logging.getLogger("vehicle.display.hdmi")


class HDMISink:
    """HDMI output controller for Pi → aftermarket head unit."""
    ...
    async def _read_edid(self) -> None:
        ...
        try:
            # On Pi, EDID is at /sys/class/drm/card0-HDMI-A-1/edid
