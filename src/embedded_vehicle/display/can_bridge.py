"""
embedded_vehicle/display/can_bridge.py
──────────────────────────────
...
The 1996 vehicle doesn't have CAN, so this module also bridges
...
logger = logging.getLogger("vehicle.display.can")
...
class CANBridge:
    ...
    async def inject_obd_snapshot(self, rpm: int | None, speed: float | None,
        ...
        Useful for 1996 vehicle which has OBD-II but not CAN.
        """
        if rpm is not None:
