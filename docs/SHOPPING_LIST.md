# Embedded-Vehicle Integration — Recommended Module Shopping List
...
- SPS30 needs 5V (can use PiV pin, draws ~60mA)
...
- HC-SR04 VCC = 5V (from PiV pin), but echo must be level-shifted to 3.3V
...
|------|----------|-------|-------|
| **12V→5V/5A buck converter** (LM2596S) | Amazon | $8 | Powers Pi + peripherals; 5A = 25W headroom |
| **12V→5V/3A USB-C PD trigger** | Amazon | $12 | Clean power, no wiring needed; plug into Pi USB-C |
...
**Fuse box mapping (1996 vehicle):**
| Circuit | Fuse | Tap for |
|---------|------|---------|
| IGN-switched | 15A | Pi power, relays |
...
12V Battery / Fuse Box
    │
    ├── Fuse Tap (IGN-switched) ──→ 12V→5V Buck ──→ Pi USB-C (5V/3A)
    │                                      │
